...
const SetBestSelling = ({productId, bestSelling,reload}) => {
      const updateSelling = async () => {
            let newSelling = bestSelling ? false : true
      ...
      return(
            <div onClick={updateSelling} className='flex items-center gap-2 cursor-pointer'> 
                  <h3 className=''>Yes</h3> 
                  <div className={`w-8 h-4 p-1 flex items-center ${bestSelling ? 'justify-start bg-green-600/60':'justify-end bg-neutral-200'} rounded-full border-2 border-black`}>
                        <div className={`w-2 h-2 bg-black rounded-full`}/>
                  </div> 
                  <h3 className=''>No</h3>
            </div>
...
export default function DetailsCard({ product, callReload }) {
      const defaultImg = JSON.parse(product.images)[0];
            ...
            <div className="flex flex-col items-start p-2 gap-1">
                  ...
                  <div className="grid grid-cols-3 gap-x-8 gap-y-2 flex-wrap w-full">
                        <div className='flex text-sm gap-2'>
                              ...
                              <h3 className=''>Rs. {formatPKR(product.price)}</h3>
                        </div>
                        <div className='flex text-sm gap-2'>
                        ...
                        <div className='flex text-sm gap-2'>
                              ...
                              <h3 className=''>{product.onSale === 1 ? <>Yes - Rs. {formatPKR(product.salePrice)}</> : 'No'}</h3>
                        </div>
                        <div className='flex text-sm gap-2'>
